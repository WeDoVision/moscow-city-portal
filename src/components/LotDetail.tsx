...
import { lotHeadline, lotInfo, lotPrice } from "@/lib/whitewill/client";
...
/**
 * Тело страницы лота. Общее для эталонного сайта (/lots/[id]) и портальных
 * маршрутов (/p/[slug]/lots/[id]). `base` задаёт префикс ссылок: "" для эталона,
 * "/p/<slug>" для портала — чтобы хлебные крошки вели в нужный каталог.
 */
export function LotDetail({ details, base = "" }: { details: LotDetails; base?: string }) {
  ...
  const homeHref = base || "/";
  const catalogHref = base ? `${base}#catalog` : "/#catalog";
    ...
    <main className="pb-24 pt-28">
      ...
      <div className="mx-auto max-w-7xl px-5 md:px-8">
        <nav className="mb-6 text-xs text-muted" aria-label="Хлебные крошки">
          ...
          <Link href={catalogHref} className="hover:text-gold">
            Каталог
          </Link>
          <span className="mx-2">/</span>
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_380px]">
          <div className="min-w-0">
            ...
            <section className="mt-8" aria-labelledby="lot-desc">
              <h2 id="lot-desc" className="font-display text-xl text-paper">
                Описание
              </h2>
