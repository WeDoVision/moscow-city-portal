...
/**
 * Тело страницы башни. Общее для эталонного сайта (/towers/[slug]) и портальных
 * маршрутов (/p/[slug]/towers/[tslug]). `base` префиксует ссылки ("" / "/p/<slug>"),
 * `portalSlug` уводит карточки лотов в портальные маршруты с темой портала.
 */
export async function TowerDetail({
  tower,
  base = "",
  portalSlug,
}: {
  ...
  const homeHref = base || "/";
  const towersHref = base || "/towers";
  const allWithFiltersHref = `${base || ""}/?towers=${tower.id}#catalog`;
      ...
      <section className="relative flex min-h-[60svh] items-end overflow-hidden">
        ...
        <div className="relative mx-auto w-full max-w-7xl px-5 pb-14 pt-40 md:px-8">
          <nav className="mb-4 text-xs text-paper/60" aria-label="Хлебные крошки">
            ...
            <Link href={towersHref} className="hover:text-gold">
              Башни
            </Link>
            <span className="mx-2">/</span>
        ...
        <section className="mt-16" aria-labelledby="tower-sale">
          ...
          <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {saleLots.moscowLotCardDTOs.slice(0, 9).map((lot) => (
              <LotCard key={lot.id} lot={lot} portalSlug={portalSlug} />
            ))}
          </div>
          {saleLots.total === 0 && (
