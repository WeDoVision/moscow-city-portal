...
Реальная геометрия района из OpenStreetMap (контуры + высоты зданий,
`scripts/build_city_data.py` → `public/data/moscow-city.json`), three.js-сцена:
скролл — пролёт камеры вокруг башен, клик по башне — карточка с ценами и
переходом к её лотам. Башни замаплены на whitewill `complex_id`.

### AI-агент подбора

Чат (кнопка «AI-подбор»): LLM с tool calling применяет фильтры того же
каталога и возвращает карточки реальных лотов в чат; кнопка «показать в
каталоге» переносит подбор в основную выдачу. Env: `OPENAI_API_KEY`
(+ опционально `OPENAI_MODEL`, по умолчанию `gpt-5.4-mini`).
